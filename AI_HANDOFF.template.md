...
Target integration: {{TARGET_PLATFORM}}

## Files

- `manifest.json`: page index and the authoritative mapping from design layers to local assets.
- `EXPORT_REPORT.md` and `export_report.json`: export coverage and resource-quality warnings; read these first.
...
1. Read `EXPORT_REPORT.md`, then read `manifest.json` before selecting or implementing a page.
...
5. Use assets only through the matching `manifest.json` entry. Use the selected platform package in `integrations/`; its `asset_map.json` maps design assets to platform resource names.
